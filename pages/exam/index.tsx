...
const Exam: NextPage = () => {
  const { remainingTime, startTimer, stopTimer, isRunning, isFinished } = useTimer({ minutes: 10, seconds: 0 });
  ...
  const getResultPoints = (data: number) => {
    setResultPoints(data);
  };
        ...
        <>
          <div className="py-10 px-5 sm:p-10 mx-auto w-5/6 sm:w-1/2 bg-slate-800 border-2 border-slate-700 rounded-lg">
              <div className="w-full flex flex-col xl:flex-row justify-between items-center">
                <p className="text-white font-bold text-2xl">{currentQuestionIndex + 1}/{numberOfQuestions}</p>
              ...
              <div className="h-max">
                <div className="grid pt-20 place-items-center">
                  ...
                  <p className="text-white text-center pt-6 px-6 text-xl font-bold">
                    EXAM FINISHED
                  </p>
                </div>
                <p className="text-white font-bold text-xl text-center pt-20 px-6 mb-40">
                  TOTAL SCORE: {resultPoints}/{numberOfQuestions}
                </p>
              </div>
          ...
          <div className={`${(isRunning && !isFinished) || revealExam ? "" : "hidden"} py-10 px-5 sm:p-10 mx-auto w-5/6 sm:w-1/2 bg-slate-800 border-2 border-slate-700 rounded-lg`}>
              <div className="w-full flex flex-col xl:flex-row justify-between items-center">
                <p className="text-white font-bold text-2xl">{currentQuestionIndex + 1}/{numberOfQuestions}</p>
                <h1 className="text-white font-bold text-3xl">PRACTICE EXAM</h1>
                <p className="text-white font-bold text-2xl">{remainingTime}</p>
              ...
              <div className="h-max">
                <QuizExamForm
                  ...
                  hideExam={() => { setRevealExam(false) }}
                />
              </div>
          </div>
      {!isRunning && !isFinished && !revealExam ? (<div className="py-10 px-5 sm:p-10 w-5/6 sm:w-1/2 bg-slate-800 border-2 border-slate-700 rounded-lg">
        <div className="w-full flex flex-col xl:flex-row justify-between items-center">
