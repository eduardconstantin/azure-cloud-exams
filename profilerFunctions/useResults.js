const useResults = (data) => {
    let points = 0;

    for (let i = 0; i < data.questions?.length; i++) {
      if (!data.answers[i] || !data.questions[i]) continue;
      const noOfAnswers = data.questions[i]?.options ? data.questions[i]?.options?.filter((el) => el.isAnswer === true).length : 0;
      if (noOfAnswers > 1) {
        let partialPoints = 0;
        ...
        if (Array.isArray(data.answers[i])) {
          for (const answer of data.answers[i] ?? []) {
            if (data.questions[i]?.options?.filter((el) => el.isAnswer === true).some((el) => el.text === answer)) {
              partialPoints = partialPoints + pointRaise;
              pointRaisedCount = pointRaisedCount + 1;
            }
            if (data.questions[i]?.options?.filter((el) => el.isAnswer === false).some((el) => el.text === answer)) {
              partialPoints = partialPoints - pointRaise;
              isOneBad = true;
            }
          }
        ...
        if (data.questions[i]?.options?.filter((el) => el.isAnswer === true)[0]?.text === data.answers[i]) {
          points = (Math.round((points + 1) * 100) / 100);
        }
      }
    }
